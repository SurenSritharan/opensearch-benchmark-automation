...
def process_batch_job(job_id: str, job: Dict[str, Any], options: Dict[str, Any], cancel_event: threading.Event = None):
    ...
    for idx, scenario in enumerate(scenarios):
        ...
        try:
            ...
            if cancel_event and cancel_event.is_set():
                logger.info(f"Batch job {job_id}: Active process killed via cancellation during {scenario_key}. Cleaning up remaining tests.")
                
                # Mark current scenario as cancelled instead of error
                job_data = get_job(job_id)
                if job_data and 'scenario_status' in job_data:
                ...
                # Forcefully break the loop so it stops spawning subsequent scenarios
                break
...
def trigger_batch_benchmark():
    ...
    Request format:
    {
        ...
        "tests": [
            ...
            {"dataset": "cohere-wiki-en-768", "scenario": "vector-search-k10"}
        ]
    }
    """
    try:
